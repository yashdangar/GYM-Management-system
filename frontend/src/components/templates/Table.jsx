...
import { Card, CardHeader, Typography, Button, CardBody, CardFooter, Avatar, Chip, Tooltip, IconButton } from "@material-tailwind/react";
import { MagnifyingGlassIcon, PlusIcon, PencilIcon,TrashIcon } from "@heroicons/react/24/solid";

function Table({ title, TABLE_HEAD, TABLE_ROWS }) {
        ...
        <div className="mb-8 flex items-center justify-between gap-8">
          <div>
            ...
            <Typography className="mt-1 text-sm text-gray-600">
              See information about all {title}
            </Typography>
          </div>
      ...
      <CardBody className="overflow-scroll px-0">
        <table className="mt-4 w-full text-left">
          <thead>
            <tr>
              {TABLE_HEAD.map((head) => (
                <th key={head} className="border-y border-blue-gray-100 bg-blue-gray-50 p-4 text-sm text-blue-gray-700">
                  {head}
                </th>
              ))}
            </tr>
